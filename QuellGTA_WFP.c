...
void PrintUsage(const char* program_name) {
    ...
    printf("  -ip, --remote-ip <IP>\n");
    printf("  按本地IPv4地址阻断 (如: 192.168.1.200)\n");  
    printf("  -lip, --local-ip <IP>\n");                   
    printf("  按远程端口阻断\n");
    printf("  -rp, --remote-port <端口>\n");
    printf("  按本地端口阻断\n");
    printf("  -lp, --local-port <端口>\n");
    printf("  按协议阻断 (tcp/udp 或 协议号)\n");
    printf("  -proto, --protocol <tcp|udp|数字>\n");
    printf("\n");
    printf("使用示例:\n");
    printf("  %s -c 创建全局规则\n", program_name);
    printf("  %s -d 清理所有本工具创建的规则并恢复网络\n", program_name);
    printf("  控制协议:\n");
    printf("    %s -c -p \"C:\\\\game.exe\" -proto tcp    # 仅阻断该进程的TCP连接\n", program_name);
    printf("    %s -c -p \"C:\\\\game.exe\" -proto udp    # 仅阻断该进程的UDP连接\n", program_name);
    printf("\n");
    printf("  控制本地IP:\n");  
    printf("    %s -c -lip 192.168.1.200                # 阻断所有使用本地192.168.1.200的连接\n", program_name);
    printf("    %s -c -p \"C:\\\\game.exe\" -lip 192.168.1.200    # 阻断该进程使用本地192.168.1.200的连接\n", program_name);
    printf("\n");
    printf("  控制本地端口:\n");
    printf("    %s -c -lp 27015                      # 阻断所有使用本地27015端口的连接\n", program_name);
    printf("    %s -c -p \"C:\\\\game.exe\" -lp 27015    # 阻断该进程使用本地27015端口的连接\n", program_name);
    printf("\n");
    printf("  控制远程端口:\n");
    printf("    %s -c -rp 443                        # 阻断所有到远程443端口(HTTPS)的连接\n", program_name);
    printf("    %s -c -p \"browser.exe\" -rp 443       # 阻断浏览器到HTTPS的连接\n", program_name);
    printf("\n");
    printf("  组合控制:\n");
    printf("    %s -c -rp 80 -lp 5000                # 阻断所有从本地5000到远程80端口的连接\n", program_name);
    printf("    %s -c -ip 192.168.1.1 -rp 53 -proto udp # 阻断所有到该IP UDP 53端口(DNS)的连接\n", program_name);
    printf("    %s -c -lip 192.168.1.200 -lp 5000 -rp 80 # 阻断从本地192.168.1.200:5000到远程80端口的连接\n", program_name); 
    printf("\n");
    printf("  精细控制 (完整示例):\n");
    printf("    %s -c -p \"C:\\\\game.exe\" -lip 192.168.1.200 -ip 1.2.3.4 -rp 27015 -lp 5000 -proto udp\n", program_name); 
    printf("        # 效果: 仅阻断 game.exe 从本地192.168.1.200:5000(UDP)连接到 1.2.3.4:27015\n");
    printf("\n");
    printf("注意事项:\n");
            ...
            *mode = MODE_CLEANUP;
            ...
            if (i + 1 < argc) {
                ...
                if (InetPtonA(AF_INET, argv[i + 1], &addr) == 1) {
                    params->remote_ip = addr.S_un.S_addr;
                    params->has_ip = TRUE;

                    printf("[解析] 远程IP原始值: %s -> 0x%08X\n",
                        argv[i + 1], params->remote_ip);
                }
                i++;
            }
        }
        else if (strcmp(argv[i], "-lip") == 0 || strcmp(argv[i], "--local-ip") == 0) { 
            if (i + 1 < argc) {
                // 解析本地IPv4地址
                struct in_addr addr;
                if (InetPtonA(AF_INET, argv[i + 1], &addr) == 1) {
                    params->local_ip = addr.S_un.S_addr;
                    params->has_local_ip = TRUE;

                    printf("[解析] 本地IP原始值: %s -> 0x%08X\n",
                        argv[i + 1], params->local_ip);
                }
                i++;
            }
        }
            ...
            if (i + 1 < argc) {
                ...
                if (port > 0 && port <= 65535) {
                    params->remote_port = (UINT16)port;
            ...
            if (i + 1 < argc) {
                int port = atoi(argv[i + 1]);
                if (port > 0 && port <= 65535) {
                    params->local_port = (UINT16)port;
                    params->has_local_port = TRUE;
                }
                i++;
            }
        }
        else if (strcmp(argv[i], "-proto") == 0 || strcmp(argv[i], "--protocol") == 0) {
            if (i + 1 < argc) {
                if (_stricmp(argv[i + 1], "tcp") == 0) {
                    params->ip_protocol = 6;
                    params->has_protocol = TRUE;
                }
                else if (_stricmp(argv[i + 1], "udp") == 0) {
                    params->ip_protocol = 17;
                    params->has_protocol = TRUE;
                }
                else {
...
BOOL CreateConditionalRule(HANDLE engine_handle, const ConditionParams* params) {
    printf("[调试] 输入参数：进程=%s, 远程IP=%s, 本地IP=%s, 远程端口=%u (原始值), 本地端口=%u, 协议指定=%d (值:%u)\n",
        params->has_process ? params->process_path : "(无)",
        params->has_ip ? "(已设置)" : "(无)",
        params->has_local_ip ? "(已设置)" : "(无)", 
    ...
    FWPM_FILTER_CONDITION condition_local_ip = { 0 };  
    ...
    FWPM_FILTER_CONDITION* all_conditions[6] = { 0 };  
    ...
    if (params->has_ip) {
        ...
        printf("[调试] 远程IP地址: 原始=0x%08X, 主机序=0x%08X\n",
    ...
    // 条件3：本地IP地址
    if (params->has_local_ip) {
        UINT32 ip_host_order = ntohl(params->local_ip);

        condition_local_ip.fieldKey = FWPM_CONDITION_IP_LOCAL_ADDRESS;
        condition_local_ip.matchType = FWP_MATCH_EQUAL;
        condition_local_ip.conditionValue.type = FWP_UINT32;
        condition_local_ip.conditionValue.uint32 = ip_host_order;

        printf("[调试] 本地IP地址: 原始=0x%08X, 主机序=0x%08X\n",
            params->local_ip, ip_host_order);

        all_conditions[condition_count] = &condition_local_ip;
        condition_count++;
    }
    ...
    // 构建最终条件数组
    FWPM_FILTER_CONDITION final_conditions[6];  // 扩容为6
